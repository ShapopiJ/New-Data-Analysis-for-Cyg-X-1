...
class Correlate():
    ...
    def read(self, radio, x_ray, band=None, band_error=None, is_bat=False):                 #feed in data files as strings
        if band == None:
            band = 1
            band_error = 2
        ...
        if is_bat:
            from swift_mjd import mjd_swift
            x_mjd = mjd_swift(x_mjd)
        ...
        x_mjd_for_cross_corr = [x_mjd[i] for i in range(len(x_flux)) if x_flux[i] > 0.0 and x_err_frac[i] < self.limit]  #No negative fluxes and no fluxes with error above limit
        x_flux_for_cross_corr = [np.log10(x_flux[i]) for i in range(len(x_flux)) if x_flux[i] > 0.0 and x_err_frac[i] < self.limit]    #want a flux larger than 0.5 and a fractional error lower than 0.5
